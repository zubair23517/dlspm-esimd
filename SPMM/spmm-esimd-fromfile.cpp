...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        printf("MSZ: %d, KSZ: %d, nnz: %d\n", MSZ_rt, KSZ_rt, nnz_rt);

        int *row_offsets = (int *)malloc((MSZ_rt + 1) * sizeof(int));
        int *column_indices = (int *)malloc(((nnz_rt + RP - 1) / RP) * RP * sizeof(int));
        float *lhs_values = (float *)malloc(((nnz_rt + RP - 1) / RP) * RP * sizeof(float));
        ...
        int *column_indices_dev = (int *)sycl::malloc_device(((nnz_rt + 2 * RP - 1) / RP) * RP * sizeof(int), dev_queue); // extend this array to ensure that the prefetching is not doing an out of bounds access
        dev_queue.memcpy(column_indices_dev, column_indices, nnz_rt * sizeof(int)).wait();
        float *values_dev = (float *)sycl::malloc_device(((nnz_rt + 2 * RP - 1) / RP) * RP * sizeof(float), dev_queue);
        ...
        const int nh_threads = ((MSZ_rt + NT - 1) / NT) * NT;

        std::cout << "#work-items = " << nh_threads << std::endl;
