...
void RunBenchmarkCase(const int MSZ, const int KSZ, const int NSZ, const double SP, sycl::queue &dev_queue)
{
    ...
    for (int loop = 0; loop < 20; ++loop)
    {
        ...
        if (NSZ % 128 == 0)
        {
            const int NT = std::max<int>(1, MIN(NSZ / 128, 4));
            const int nh_threads = ((MSZ + NT - 1) / NT) * NT;
        ...
        {
            const int NT = std::max<int>(1, MIN(NSZ / 64, 4));
            const int nh_threads = ((MSZ + NT - 1) / NT) * NT;
            e0 = dev_queue.parallel_for(
                sycl::nd_range<1>(nh_threads, NT), [=](sycl::nd_item<1> item, sycl::kernel_handler kh) SYCL_ESIMD_KERNEL
                { SpmmEsimd<64>(
        ...
        {
            const int NT = std::max<int>(1, MIN(NSZ / 32, 4));
            const int nh_threads = ((MSZ + NT - 1) / NT) * NT;
            e0 = dev_queue.parallel_for(
                sycl::nd_range<1>(nh_threads, NT), [=](sycl::nd_item<1> item, sycl::kernel_handler kh) SYCL_ESIMD_KERNEL
                { SpmmEsimd<32>(
        ...
        {
            const int NT = std::max<int>(1, MIN(NSZ / 16, 4));
            const int nh_threads = ((MSZ + NT - 1) / NT) * NT;
            e0 = dev_queue.parallel_for(
                sycl::nd_range<1>(nh_threads, NT), [=](sycl::nd_item<1> item, sycl::kernel_handler kh) SYCL_ESIMD_KERNEL
                { SpmmEsimd<16>(
