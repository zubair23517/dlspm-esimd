...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        int *column_indices_dev = (int *)sycl::malloc_device(nnz_rt * sizeof(int), dev_queue); // extend this array to ensure that the prefetching is not doing an out of bounds access
        dev_queue.memcpy(column_indices_dev, column_indices, nnz_rt * sizeof(int)).wait();
        float *values_dev = (float *)sycl::malloc_device(nnz_rt * sizeof(float), dev_queue);
        ...
        for (int loop = 0; loop < 20; loop++)
        {
            ...
            if (NSZ_rt % 128 == 0)
            {
                const int NT = std::max<int>(1, MIN(NSZ_rt / 128, 4));
                const int nBlks = (NSZ_rt + (NT * 128) - 1) / (NT * 128);
            ...
            {
                const int NT = std::max<int>(1, MIN(NSZ_rt / 64, 4));
                const int nBlks = (NSZ_rt + (NT * 64) - 1) / (NT * 64);
                const int nh_threads = MSZ_rt * nBlks * NT;
                e0 = dev_queue.parallel_for(
                    sycl::nd_range<1>(nh_threads, NT), [=](sycl::nd_item<1> item, sycl::kernel_handler kh) SYCL_ESIMD_KERNEL
                    { SpmmEsimd<64>(
            ...
            {
                const int NT = std::max<int>(1, MIN(NSZ_rt / 32, 4));
                const int nBlks = (NSZ_rt + (NT * 32) - 1) / (NT * 32);
                const int nh_threads = MSZ_rt * nBlks * NT;
                e0 = dev_queue.parallel_for(
                    sycl::nd_range<1>(nh_threads, NT), [=](sycl::nd_item<1> item, sycl::kernel_handler kh) SYCL_ESIMD_KERNEL
                    { SpmmEsimd<32>(
            ...
            {
                const int NT = std::max<int>(1, MIN(NSZ_rt / 16, 4));
                const int nBlks = (NSZ_rt + (NT * 16) - 1) / (NT * 16);
                const int nh_threads = MSZ_rt * nBlks * NT;
                e0 = dev_queue.parallel_for(
                    sycl::nd_range<1>(nh_threads, NT), [=](sycl::nd_item<1> item, sycl::kernel_handler kh) SYCL_ESIMD_KERNEL
                    { SpmmEsimd<16>(
