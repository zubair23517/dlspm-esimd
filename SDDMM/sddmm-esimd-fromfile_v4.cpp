...
#include "sddmm-esimd-fromfile_v4.hpp"
...
void Sddmm(const int *row_offsets, const int *column_indices,
           const float *lhs_matrix, const float *rhs_matrix,
           float *output_values, const int MSZ, const int NSZ)
{
    for (int i = 0; i < MSZ; ++i)
    {
...
int main(int argc, char **argv)
{
    ...
    std::cout << "Filename = " << filename << ", batchsize (=NSZ) = " << NSZ
              << std::endl;
    ...
    // create a random lhs_matrix array with size m*k and initialize it with
    // random values using rand() function
    ...
    // create a random rhs_matrix array with size n*k and initialize it with
    // random values using rand() function
    ...
    Sddmm(row_offsets, column_indices, lhs_matrix, rhs_matrix, output_cpu, MSZ,
          NSZ);
    ...
    int *row_offsets_dev =
        (int *)sycl::malloc_device((MSZ + 1) * sizeof(int), dev_queue);
    // copy data from host to device
    dev_queue.memcpy(row_offsets_dev, row_offsets, (MSZ + 1) * sizeof(int));
    int *column_indices_dev =
        (int *)sycl::malloc_device((nnz + 2 * 16) * sizeof(int), dev_queue);
    dev_queue.memcpy(column_indices_dev, column_indices, nnz * sizeof(int));
    float *lhs_matrix_dev = (float *)sycl::malloc_device(
        (MSZ * NSZ + 128) * sizeof(float), dev_queue);
    dev_queue.memcpy(lhs_matrix_dev, lhs_matrix, MSZ * NSZ * sizeof(float));
    float *rhs_matrix_dev = (float *)sycl::malloc_device(
        (KSZ * NSZ + 128) * sizeof(float), dev_queue);
    dev_queue.memcpy(rhs_matrix_dev, rhs_matrix, KSZ * NSZ * sizeof(float));
    float *output_dev =
        (float *)sycl::malloc_device(nnz * sizeof(float), dev_queue);
    ...
    const int ideal_NT = smallK::ComputeNT(MSZ);
    const int tmp_NSZC = (NSZ + ideal_NT - 1) / ideal_NT;
    int NSZC = 8;
    for (; NSZC <= 64; NSZC = NSZC << 1)
    {
        if (tmp_NSZC <= NSZC)
            break;
    }
    std::cout << "NSZC = " << NSZC << std::endl;

    const int total_chunks = (NSZ + NSZC - 1) / NSZC;
    const int NT = MIN(total_chunks, ideal_NT);
...
#pragma forceinline recursive
        event e0 = smallK::Launcher(row_offsets_dev, column_indices_dev, lhs_matrix_dev,
                                    rhs_matrix_dev, output_dev, NSZ, NT, nh_threads, NSZC, dev_queue);

        tt = (e0.template get_profiling_info<info::event_profiling::command_end>() -
              e0.template get_profiling_info<
    ...
    double bytes_accessed =
        ((static_cast<double>(nh_threads) * NSZ + static_cast<double>(NSZ) * nnz +
          static_cast<double>(nnz)) *
         sizeof(float)); // every A row is only accessed once for each thread.
                         // Every Col of B is accesses once for each nnz. There
                         // are probably also some cache effects. Every value in C
                         // is written to once
    double calc_bw;
    ...
    csv_file << MSZ << "," << KSZ << "," << NSZ << ","
             << (double)nnz / (MSZ * KSZ) << "," << gflops1 << "," << flops << ","
             << max_diff << std::endl;
    csv_file.close();
#endif
