...
void copy_device(float *big_array1, float *big_array2, int nnz, sycl::nd_item<1> item)
{
    int i = item.get_global_id(0);
    if (i < nnz)
    {
        big_array2[i] = big_array1[i];
    }
}
...
int main(int argc, char **argv)
{
    ...
    float *big_arrayh = (float *)malloc(nnz * 10 * sizeof(float));
    float *big_array1 = (float *)sycl::malloc_device(nnz * 10 * sizeof(float), dev_queue);
    float *big_array2 = (float *)sycl::malloc_device(nnz * 10 * sizeof(float), dev_queue);
    ...
    {
        dev_queue.memset(big_array1, 0, nnz * 10 * sizeof(float));
        dev_queue.memset(output_dev, 0, nnz * sizeof(float));
        event e1;
        e1 = dev_queue.parallel_for(
            sycl::nd_range<1>(10*nnz, 10), [=](sycl::nd_item<1> item) SYCL_ESIMD_KERNEL
            {
               copy_device(big_array1, big_array2, nnz, item); });
               dev_queue.wait();        
        event e0;
        if (NT == 1)
...
#pragma forceinline recursive
    ...
    float opcount = static_cast<double>(nnz) * (2.0 * NSZ - 1.0);
    double gflops1 = (opcount) / (exec_time * 1.0E+6);

    printf("opcount: %20.4f GFLOPS:  %8.2f\n", opcount, gflops1);
    ...
    dev_queue.memcpy(big_arrayh, big_array2, nnz * 10 * sizeof(float)).wait();
    printf(" big_arrayh[0] = %f\n", big_arrayh[0]);
