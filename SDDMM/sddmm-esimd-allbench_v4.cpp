...
static inline void RunBenchmarkCase(const int MSZ, const int KSZ, const int NSZ, const double SP, sycl::queue &dev_queue)
{
    ...
    const int mnnz = (1.01 * MSZ * KSZ * SP) + 1000;
    ...
    int *column_indices_dev = (int *)sycl::malloc_device(nnz * sizeof(int) + 1000, dev_queue);
    dev_queue.memcpy(column_indices_dev, column_indices, nnz * sizeof(int));
    float *lhs_matrix_dev = (float *)sycl::malloc_device(MSZ * NSZ * sizeof(float) + 1000, dev_queue);
    dev_queue.memcpy(lhs_matrix_dev, lhs_matrix, MSZ * NSZ * sizeof(float));
    float *rhs_matrix_dev = (float *)sycl::malloc_device(KSZ * NSZ * sizeof(float) + 1000, dev_queue);
...
#pragma forceinline recursive
        event e0 = smallK::Launcher(row_offsets_dev,
                                    column_indices_dev, lhs_matrix_dev,
                                    rhs_matrix_dev, output_dev,
                                    NSZ, NT, nh_threads, NSZC, dev_queue);
