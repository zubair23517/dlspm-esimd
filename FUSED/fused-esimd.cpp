...
// ESIMD Code
void FusedEsimd(const int *a_row_offsets,
                const int *a_column_indices, const float *c,
                const float *b, const float *d, float *e, sycl::nd_item<1> item)
{
    ...
    simd<float, NSZ> reg_left = my_lsc_block_load<float, NSZ>(c + i * NSZ);
    simd<int, 2> lrowptr2 = lsc_block_load<int, 2>(a_row_offsets + i);
    const int nnzr = lrowptr2.select<1, 1>(1) - lrowptr2.select<1, 1>(0);
    simd<float, VLC> a_row;
    simd<float, NSZ> e_row;
    ...
    {
        col_ida = lsc_block_load<int, VLC>(a_column_indices + idxb);
        col_ida = col_ida * NSZ;
#pragma unroll
        for (int l0 = 0; l0 < VLC; l0++)
        {
            simd<float, NSZ> reg_right = my_lsc_block_load<float, NSZ>(b + col_ida.select<1, 1>(l0));
...
#pragma unroll
    ...
    idxb = lrowptr2.select<1, 1>(0) + (nchunks - 1) * VLC;

    simd<unsigned, VLC> sa(0, 1);
    simd_mask<VLC> mask = sa < lcsz;

    simd<uint32_t, VLC> offset(0, sizeof(int));
    col_ida = 0;
...
#pragma unroll
    ...
    // lsc_scatter(a_values + idxb, offsets, a_row, mask);
    // my_lsc_scatter<float, VLC>(a_values + idxb, simd<uint32_t, VLC>(0, sizeof(float)), a_row, lcsz);
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < MSZ; i++)
    {
        ...
        for (int j = 0; j < KSZ; j++)
        {
            if ((static_cast<float>(rand()) / static_cast<float>(RAND_MAX) < SP))
            {
                ...
                // values[a_row_offsets[i + 1]] = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
                a_row_offsets[i + 1]++;
            }
        }
    }
    ...
    int *a_column_indices_dev = (int *)sycl::malloc_device(nnz * sizeof(int), dev_queue);
    dev_queue.memcpy(a_column_indices_dev, a_column_indices, nnz * sizeof(int));
    float *c_dev = (float *)sycl::malloc_device(MSZ * NSZ * sizeof(float), dev_queue);
    dev_queue.memcpy(c_dev, c, MSZ * NSZ * sizeof(float));
    float *b_dev = (float *)sycl::malloc_device(KSZ * NSZ * sizeof(float), dev_queue);
    dev_queue.memcpy(b_dev, b, KSZ * NSZ * sizeof(float));

    float *d_dev = (float *)sycl::malloc_device(KSZ * NSZ * sizeof(float), dev_queue);
    dev_queue.memcpy(d_dev, d, KSZ * NSZ * sizeof(float));

    float *e_dev = (float *)sycl::malloc_device(MSZ * NSZ * sizeof(float), dev_queue);
